...
def _apply_metadata(
    ...
    *,
    ...
    _apply_material_type(material, root)
...
def _apply_material_type(material: Material, root: ET.Element) -> None:
    """Apply verified type-specific behavior and report unsupported specializations."""
    material_type = _text(root, "TypeV5") or _text(root, "Type")
    if material_type == "GLASS":
        ...
        material.alpha = 1.0
    elif material_type not in {"", "GENERIC", "SELF_ILLUMINATED"}:
        logger.warning(
            "Unsupported Enscape material type %r for %r; importing supported properties only",
            material_type,
            material.name,
        )
